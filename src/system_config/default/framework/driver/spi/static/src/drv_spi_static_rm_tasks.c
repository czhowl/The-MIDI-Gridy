...
int32_t DRV_SPI0_MasterRMSend16BitPolled( struct DRV_SPI_OBJ * pDrvObj )
{
    ...
    /* Transmit the data & update the counts */
        PLIB_SPI_BufferWrite16bit(SPI_ID_1, ((uint16_t*)currentJob->txBuffer)[currentJob->dataTxed>>1]);
    ...
    else
    {
        ...
        PLIB_SPI_BufferWrite16bit(SPI_ID_1, 0xffff);
...
int32_t DRV_SPI0_MasterRMReceive16BitPolled( struct DRV_SPI_OBJ * pDrvObj )
{
    register DRV_SPI_JOB_OBJECT * currentJob = pDrvObj->currentJob;
    ...
    if (PLIB_SPI_ReceiverBufferIsFull(SPI_ID_1))
    {
        ...
        if (currentJob->dataLeftToRx != 0)
        {
            ...
            ((uint16_t*)(currentJob->rxBuffer))[currentJob->dataRxed>>1] = PLIB_SPI_BufferRead16bit(SPI_ID_1);
